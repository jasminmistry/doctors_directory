...
export const getAllProducts = cache(async (): Promise<Product[]> => {
  return withQueryCache('products:all-list', async () => {
...
export const getProductBySlug = cache(async (slug: string): Promise<Product | null> => {
  return withQueryCache(`product:slug:${slug}`, async () => {
...
export const getProductsByCategory = cache(async (categorySlug: string): Promise<Product[]> => {
  return withQueryCache(`products:category:${categorySlug}`, async () => {
    const rows = await prisma.product.findMany({
...
export const getProductsByBrand = cache(async (brand: string): Promise<Product[]> => {
  return withQueryCache(`products:brand:${brand}`, async () => {
    const rows = await prisma.product.findMany({
      where: { brand },
      select: SELECT_FOR_LIST,
      orderBy: { productName: 'asc' },
...
export const getAllBrands = cache(async (): Promise<string[]> => {
  return withQueryCache('products:brands', async () => {
    const rows = await prisma.product.findMany({
      select: { brand: true },
      distinct: ['brand'],
...
export const getAllCategorySlugs = cache(async (): Promise<string[]> => {
  return withQueryCache('products:categories', async () => {
    const rows = await prisma.product.findMany({
